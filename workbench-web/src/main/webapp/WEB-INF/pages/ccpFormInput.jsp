...
<script type="text/javascript">
	...
	$(document).ready(function(){
		$('#yBox').click(function(){  $('#mail').show();});
		$('#nBox').click(function(){   $('#mail').hide();});
	});
</script>
</head>
<body>
 ...
 -->
		...
		<div>
			...
			<h1 id="display1" style="display: none;">Successfully Downloaded</h1>
			<h1 id="display2" style="display: none; color: RED;">Dowload Failure Please Contact The Developer</h1>
		</div>
							...
							<c:forEach items="${parameters}" var="parameter">
								<tr>
									...
									<td><c:if
										...
										</c:if> <c:if test="${parameter.flexVSName eq 'FND_STANDARD_DATE' || parameter.flexVSName eq 'PER_DATES'
															  || parameter.flexVSName eq 'PER_DATES_STANDARD'}">
											<input type="text" name="${parameter.prompt}" class="tcal" value="" />
											<c:if test="${parameter.required_flag =='Y'}">
												<font color="red">*</font>
											</c:if>
										</c:if></td>
									...
									<c:if test="${parameter.default_Value eq  'PER_BUSINESS_GROUP_ID'}">
										<input type="hidden" name="${parameter.prompt}"
											value="${bgID}">
									</c:if>
